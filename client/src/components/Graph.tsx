...
ChartJS.register(CategoryScale, LinearScale, PointElement, Legend, Tooltip, LineElement);
    ...
    useEffect(() => {
        async function fetchStockData() {
            const responses = await Promise.all(
                tickers.map((ticker) => fetch(`http://localhost:5000/api/getData/${ticker}`)),
            );
        ...
        async function fetchCompanyData() {
            const companyData = await fetch("http://localhost:5000/api/getCompanyData/")
                .then((res) => res.json())
                .then((data) => {
                    return data;
                });
